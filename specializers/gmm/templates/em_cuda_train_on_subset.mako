...
boost::python::tuple em_cuda_train_on_subset${'_'+'_'.join(param_val_list)} (int num_components, 
                                                                             int num_dimensions, 
                                                                             int num_events,
                                                                             int num_indices,
                                                                             int min_iters,
...
%if covar_version_name.upper() in ['2B','V2B','_V2B']:
    ...
    ${tempbuff_type_name} *zeroR_2b =
       (${tempbuff_type_name}*)malloc(sizeof(${tempbuff_type_name}) *
                     num_dimensions*num_dimensions*num_components);

    for(int i = 0; i<num_dimensions * num_dimensions*num_components; i++) {
        zeroR_2b[i] = 0;
    }
    CUDA_SAFE_CALL(cudaMalloc((void**) &(temp_buffer_2b),
                              sizeof(${tempbuff_type_name}) *
                              num_dimensions *
                              num_dimensions *
                              num_components));
    CUDA_SAFE_CALL(cudaMemcpy(temp_buffer_2b,
                              zeroR_2b,
                              sizeof(${tempbuff_type_name}) *
                              num_dimensions *
                              num_dimensions *
...
%endif
  ...
  constants_kernel_launch${'_'+'_'.join(param_val_list)}(d_components,
                                                         num_components,
                                                         num_dimensions);
  ...
  compute_average_variance_launch${'_'+'_'.join(param_val_list)}(d_fcs_data_by_event,
                                                                 d_components,
                                                                 num_dimensions,
                                                                 num_components,
                                                                 num_events);
  // Calculate an epsilon value
  float epsilon = (1 + num_dimensions + 0.5 *(num_dimensions + 1) * num_dimensions) *
                  log((float)num_events * num_dimensions) * 0.0001;
  int iters;
  ...
  float* d_likelihoods;
  CUDA_SAFE_CALL(cudaMalloc((void**) &d_likelihoods,
                            sizeof(float) * ${num_blocks_estep}));
  ...
  //while(iters < min_iters || (iters < max_iters && change > epsilon)) {
    ...
    estep1_launch${'_'+'_'.join(param_val_list)}(d_fcs_data_by_dimension,
                                                 ...
                                                 num_events,
                                                 d_loglikelihoods);
    estep2_launch${'_'+'_'.join(param_val_list)}(d_fcs_data_by_dimension,
                                                 d_components,
                                                 d_component_memberships,
                                                 num_dimensions,
                                                 num_components,
    ...
    CUDA_SAFE_CALL(cudaMemcpy(likelihoods,
                              d_likelihoods,
                              sizeof(float) * ${num_blocks_estep},
    ...
    mstep_N_launch_idx${'_'+'_'.join(param_val_list)}(d_fcs_data_by_event,
                                                     d_index_list,
                                                     num_indices,
                                                     d_components,
                                                     d_component_memberships,
    ...
    mstep_means_launch_idx${'_'+'_'.join(param_val_list)}(d_fcs_data_by_dimension,
                                                          d_index_list,
                                                          num_indices,
                                                          d_components,
                                                          d_component_memberships,
...
%endif
    ...
    mstep_covar_launch_idx${'_'+'_'.join(param_val_list)}(d_fcs_data_by_dimension,
                                                          d_fcs_data_by_event,
                                                          d_index_list,
                                                          num_indices,
                                                          d_components,
