void seed_covars${'_'+'_'.join(param_val_list)}(components_t* components,
                                                float* fcs_data,
                                                float* means,
                                                int num_dimensions,
                                                int num_events,
    ...
    cilk_for(int i = 0; i < num_dimensions*num_dimensions; i++) {
      ...
      if(row==col) {
        components->R[row*num_dimensions+col] /=
            (float) (num_events -1);
        components->R[row*num_dimensions+col] -=
            ((float)(num_events)*means[row]*means[row]) / (float)(num_events-1);
        components->R[row*num_dimensions+col] /=
...
void average_variance${'_'+'_'.join(param_val_list)}(float* fcs_data,
                                                     float* means,
                                                     int num_dimensions,
                                                     int num_events,
                                                     float* avgvar) {
...
void constants${'_'+'_'.join(param_val_list)}(components_t* components,
                                              int M, int D) {
...
void seed_components${'_'+'_'.join(param_val_list)}(float *data_by_event,
    ...
    seed_covars${'_'+'_'.join(param_val_list)}(components,
                                               ...
                                               &avgvar,
                                               num_components);
    average_variance${'_'+'_'.join(param_val_list)}(data_by_event,
                                                    means,
                                                    num_dimensions,
                                                    num_events,
                                                    &avgvar);    
    ...
    for(int c = 1; c < num_components; c++) {
        memcpy(&components->means[c*num_dimensions],
               &data_by_event[((int)(c * seed)) * num_dimensions],
...
void compute_average_variance${'_'+'_'.join(param_val_list)}(float* fcs_data,
                                                             components_t* components,
                                                             int num_dimensions,
                                                             int num_components,
                                                             int num_events) {
...
void estep1${'_'+'_'.join(param_val_list)}(float* data,
                                           components_t* components,
...
%endif
            component_memberships[m*N+n] =
                (component_pi > 0.0f) ? -0.5 * like + component_constant + logf(component_pi) :
                MINVALUEFORMINUSLOG;
        }
    }
...
float estep2_events${'_'+'_'.join(param_val_list)}(components_t* components,
                                                   float* component_memberships,
                                                   int M, int n, int N) {
	...
	for(int m = 0; m < M; m++) {
	    component_memberships[m*N+n] =
            expf(component_memberships[m*N+n] - temp);
	}
	return thread_likelihood;
}

void estep2${'_'+'_'.join(param_val_list)}(float* data,
                                           components_t* components,
                                           float* component_memberships,
                                           int D, int M, int N,
                                           float* likelihood) {
    cilk::reducer_opadd<float> total(0.0f);
    cilk_for(int n=0; n < N; n++) {
        total += estep2_events${'_'+'_'.join(param_val_list)}(components,
...
void mstep_mean${'_'+'_'.join(param_val_list)}(float* data,
                                               components_t* components,
                                               float* component_memberships,
                                               int D, int M, int N) {
    cilk_for(int m = 0; m < M; m++) {
...
void mstep_mean_idx${'_'+'_'.join(param_val_list)}(float* data_by_dimension,
                                                   int* indices,
                                                   int num_indices,
                                                   components_t* components,
                                                   float* component_memberships,
    ...
    cilk_for(int m = 0; m < M; m++) {
        for(int d = 0; d < D; d++) {
	        ...
	        for(int index = 0; index < num_indices; index++) {
                int n = indices[index];
		        components->means[m*D+d] += data_by_dimension[d*N+n] *
                                            component_memberships[m*N+n];
	        }
	        components->means[m*D+d] /= components->N[m];
        }
...
void mstep_n${'_'+'_'.join(param_val_list)}(float* data,
                                            components_t* components,
                                            float* component_memberships,
                                            int D, int M, int N) {
    cilk_for(int m = 0; m < M; m++) {
...
void mstep_n_idx${'_'+'_'.join(param_val_list)}(float* data,
                                                int* indices,
                                                int num_indices,
                                                components_t* components,
                                                float* component_memberships,
...
void mstep_covar${'_'+'_'.join(param_val_list)}(float* data,
                                                components_t* components,
                                                float* component_memberships,
                                                int D, int M, int N) {
    cilk_for(int m = 0; m < M; m++) {
...
%endif
                float sum = 0.0;
                for(int n = 0; n < N; n++) {
                    sum += (data[i*N+n] - means[i]) *
                           (data[j*N+n]-means[j]) * component_memberships[m*N+n];
                }
...
void mstep_covar_idx${'_'+'_'.join(param_val_list)}(float* data_by_dimension,
                                                    float* data_by_event,
                                                    int* indices,
                                                    int num_indices,
                                                    components_t* components,
...
%endif
                ...
                for(int index=0; index < num_indices; index++) {
                    int n = indices[index];
                    sum += (data_by_dimension[i*N+n] - means[i]) *
                           (data_by_dimension[j*N+n] - means[j]) *
                           component_memberships[m*N+n];
                }
