...
void copy_component(components_t* dest, int c_dest, components_t* src,
                    int c_src, int num_dimensions);
void add_components(components_t* components, int c1, int c2,
                    components_t* temp_component, int num_dimensions);
float component_distance(components_t* components, int c1, int c2,
                         components_t* temp_component, int num_dimensions);

//=== Helper function prototypes ===
void invert_cpu(float* data, int actualsize,
                float* log_determinant);
int invert_matrix(float* a, int n,
                  float* determinant);
...
components_t* alloc_temp_component_on_CPU(int num_dimensions) {
    ...
    scratch_component->means = (float*)malloc(sizeof(float) *
                                num_dimensions);
    scratch_component->R = (float*)malloc(sizeof(float) *
                            num_dimensions * num_dimensions);
    scratch_component->Rinv = (float*)malloc(sizeof(float) *
                               num_dimensions * num_dimensions);
...
void alloc_events_on_CPU(PyObject* input_data) {
    ...
    fcs_data_by_dimension  = (float*)malloc(sizeof(float) *
                              num_events * num_dimensions);
    
    for(int e = 0; e < num_events; e++) {
        for(int d = 0; d < num_dimensions; d++) {
            fcs_data_by_dimension[d * num_events + e] =
                fcs_data_by_event[e * num_dimensions + d];
        }
    }
}
...
void alloc_events_from_index_on_CPU(PyObject* input_data, PyObject* indices,
                                    int num_indices, int num_dimensions) {
    fcs_data_by_event = (float*)malloc(num_indices * num_dimensions * sizeof(int));
    for(int i = 0; i<num_indices; i++) {
        for(int d = 0; d<num_dimensions; d++) {
            fcs_data_by_event[i * num_dimensions + d] =
                ((float*)PyArray_DATA(input_data))[((int*)PyArray_DATA(indices))[i] *
                                                   num_dimensions + d];
        }
    }
...
void alloc_components_on_CPU(int M, int D, PyObject* weights,
                             PyObject* means, PyObject* covars,
                             PyObject* comp_probs) {
...
void relink_components_on_CPU(PyObject* weights,
                              PyObject* means,
                              PyObject* covars) {
    components.pi = ((float*)PyArray_DATA(weights));
    components.means = ((float*)PyArray_DATA(means));
...
void alloc_evals_on_CPU(PyObject* component_mem_np_arr,
                        PyObject* loglikelihoods_np_arr){
...
float Log_Add(float log_a, float log_b)
{
    ...
    if((log_b - log_a) <= MINVALUEFORMINUSLOG){ 
        ...
        result = log_a + (float)(lut_log(1.0 +
                 (double)(exp((double)(log_b - log_a))),
                 LOOKUP_TABLE,N_LOOKUP_SIZE));
    }
    return result;
}
...
double Log_Likelihood(int DIM, int m, float *feature, float *means, float *covars, float CP) {
    ...
    return((double) - 0.5 * (y + CP));
}

float Log_Likelihood_KL(float* feature, int DIM, int gmm_M,
                        float* gmm_weights, float* gmm_means,
                        float* gmm_covars, float* gmm_CP) {

    float log_lkld= MINVALUEFORMINUSLOG ,aux;
    for(int i=0;i<gmm_M;i++) {
        aux = lut_log(gmm_weights[i], LOOKUP_TABLE, N_LOOKUP_SIZE)
              + Log_Likelihood(DIM, i, feature, gmm_means, gmm_covars, gmm_CP[i]);
...
float compute_KL_distance(int DIM, int gmm1_M, int gmm2_M,
                          PyObject* gmm1_weights_in, PyObject* gmm1_means_in,
                          PyObject* gmm1_covars_in, PyObject* gmm1_CP_in,
                          PyObject* gmm2_weights_in, PyObject* gmm2_means_in,
                          PyObject* gmm2_covars_in, PyObject* gmm2_CP_in) {
    float aux;
    ...
    for(int i = 0; i < gmm1_M; i++) {
        ...
        for(int k = 0; k < DIM; k++) {
            ...
            log_g1 += Log_Likelihood_KL(point_a, DIM, gmm2_M, gmm2_weights,
                                        gmm2_means, gmm2_covars, gmm2_CP) +
                      Log_Likelihood_KL(point_b, DIM, gmm2_M, gmm2_weights,
                                        gmm2_means, gmm2_covars, gmm2_CP);
            log_f1 += Log_Likelihood_KL(point_a, DIM, gmm1_M, gmm1_weights,
                                        gmm1_means, gmm1_covars, gmm1_CP) +
                      Log_Likelihood_KL(point_b, DIM, gmm1_M, gmm1_weights,
                                        gmm1_means, gmm1_covars, gmm1_CP);
        }
    ...
    for(int i = 0; i < gmm2_M; i++) {
        ...
        for(int k = 0; k < DIM; k++) {
            ...
            log_g2 += Log_Likelihood_KL(point_a, DIM, gmm2_M, gmm2_weights,
                                        gmm2_means, gmm2_covars, gmm2_CP) +
                      Log_Likelihood_KL(point_b, DIM, gmm2_M, gmm2_weights,
                                        gmm2_means, gmm2_covars, gmm2_CP);
            log_f2 += Log_Likelihood_KL(point_a, DIM, gmm1_M, gmm1_weights,
                                        gmm1_means, gmm1_covars, gmm1_CP) +
                      Log_Likelihood_KL(point_b, DIM, gmm1_M, gmm1_weights,
                                        gmm1_means, gmm1_covars, gmm1_CP);
        }
...
void merge_components(int min_c1, int min_c2, components_t* min_component,
                      int num_components, int num_dimensions) {
...
float component_distance(components_t* components, int c1, int c2,
    ...
    // this updates pi,means,R,N and stores in temp_component
    add_components(components,c1,c2,temp_component,num_dimensions);
    
    return components->N[c1] * components->constant[c1] +
           components->N[c2] * components->constant[c2] -
           temp_component->N[0] * temp_component->constant[0];
}
...
void add_components(components_t* components, int c1, int c2,
    ...
    for(int i=0; i<num_dimensions;i++) {
        temp_component->means[i] = wt1 * components->means[c1*num_dimensions+i] +
                                   wt2 * components->means[c2*num_dimensions+i];
    }
    ...
    for(int i=0; i<num_dimensions; i++) {
        for(int j=i; j<num_dimensions; j++) {
            ...
            temp_component->R[i*num_dimensions+j] = ((temp_component->means[i] -
                                                      components->means[c1*num_dimensions+i]) *
                                                   (temp_component->means[j] -
                                                     components->means[c1*num_dimensions+j]) + 
                                                   components->R[c1*num_dimensions*num_dimensions +
                                                                 i * num_dimensions+j]) * wt1;
            // Add R contribution from component2
            temp_component->R[i*num_dimensions+j] += ((temp_component->means[i] -
                                                       components->means[c2*num_dimensions+i]) *
                                                     (temp_component->means[j] -
                                                      components->means[c2*num_dimensions+j]) +
                                                     components->R[c2*num_dimensions*num_dimensions +
                                                                   i*num_dimensions+j])*wt2;
...
void copy_component(components_t* dest, int c_dest,
                    components_t* src, int c_src,
                    int num_dimensions) {
    ...
    memcpy(&(dest->means[c_dest * num_dimensions]),
           &(src->means[c_src*num_dimensions]),
           sizeof(float)*num_dimensions);
    memcpy(&(dest->R[c_dest*num_dimensions*num_dimensions]),
           &(src->R[c_src*num_dimensions*num_dimensions]),
           sizeof(float)*num_dimensions*num_dimensions);
    memcpy(&(dest->Rinv[c_dest*num_dimensions*num_dimensions]),
...
void print_evals(float* component_memberships,
                 float* loglikelihoods,
                 int num_events,
                 int num_components){
    for(int m = 0; m < num_components; m++){
...
void print_components(components_t* components,
                      int num_components,
                      int num_dimensions){
    printf("===============\n");
    for(int m = 0; m < num_components; m++){
    ...
    for(int m = 0; m < num_components; m++){
        for(int d = 0; d < num_dimensions; d++) {
            for(int d2 = 0; d2 < num_dimensions; d2++) {
                printf("%0.8f ",
                components->R[m * num_dimensions * num_dimensions +
                              d * num_dimensions + d2]);
            }
        }
        printf("\n");
    ...
    for(int m = 0; m < num_components; m++){
        for(int d = 0; d < num_dimensions; d++) {
            for(int d2 = 0; d2 < num_dimensions; d2++) {
                printf("%0.8f ",
                components->Rinv[m * num_dimensions * num_dimensions +
                                 d * num_dimensions + d2]);
            }
        }
        printf("\n");
